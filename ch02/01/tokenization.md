...
# 终于正确了
result = [x if x > 0 else -x for x in numbers]
# 等等，Python 的三元表达式是：value_if_true if condition else value_if_false
...
# JavaScript 等价
const result = numbers.map(x => x > 0 ? x : -x);
```
...
- [NLTK 分词介绍](https://www.nltk.org/book/ch03.html)
